...
ExaBGP is running as a docker container and exposing TCP port 179 so we can peer out intended edge/transit devices to the flowspec controller. In our case, we will peer using an XR device which will also be configured as a route-reflector for the flowspec v4 address family, which will reflect the flowspec updates to it's clients (transit 1 and transit 2). The docker-compose file can be found [here](https://github.com/BSpendlove/flowspec-v4-example/blob/master/docker-compose.yml). Some initial configuration has been added to the exabgp.conf which enables the IPv4 unicast and flowspec families towards 10.255.255.1 (loopback of the Route Reflector), also I have attached a process to the exabgp application which is a simply flask app. Currently this app performs no valdiation and allows a simple POST request for a specific URL and attempts to find 2 keywords within the POST body which are, command and neighbor.
